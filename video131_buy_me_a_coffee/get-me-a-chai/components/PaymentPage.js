...
const PaymentPage = ({ username }) => {
    ...
    const getData = async () => {
        ...
        // Convert payments to plain objects with string ids
        const safePayments = dbpayments.map(p => ({
            ...p,
            _id: p._id?.toString?.() || p._id,
            // Add more conversions if you have other ObjectId fields
        }))
          ...
          <div className="payment flex gap-8 w-full max-w-5xl mt-11 flex-col md:flex-row">
            ...
            <div className="makePayment w-full md:w-1/2 bg-gray-900/80 rounded-2xl text-white px-2 md:p-10 shadow-xl border border-gray-800 backdrop-blur-md">
              ...
              <div className="flex gap-4 flex-col">
                ...
                <input onChange={handleChange} value={paymentform.amount} name="amount" type="text" className="w-full p-3 rounded-lg bg-gray-800 text-gray-100 border border-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-600" placeholder="Enter Amount" />
                <button onClick={() => pay(Number.parseInt(paymentform.amount) * 100)} type="button" className="text-gray-100 bg-gray-800 hover:bg-gray-700 focus:ring-2 focus:outline-none focus:ring-gray-600 font-medium rounded-lg text-base px-5 py-2.5 text-center mb-2 border border-gray-700 transition-all duration-150 disabled:bg-slate-600 disabled:from-purple-100" disabled={paymentform.name?.length < 3 || paymentform.message?.length < 4 || paymentform.amount?.length < 1}>Pay</button>
              </div>
