...
const Navbar = () => {
    ...
    if (inputValue.trim() === "") {
        toast.error("Please enter a handle to view your links.");
        return;
    }

    try {
        const res = await fetch(`/api/check-handle?handle=${inputValue}`);
        const data = await res.json();

        if (!data.exists) {
            toast.error("That handle is not made.");
            return;
        }
    ...
    return ( <>{showNavbar && <nav className='z-2 bg-white w-[90vw] flex justify-between fixed top-12 right-[5vw] rounded-full p-3 pl-6'>
            ...
            <div className='flex'>
              <input className='rounded-full p-4 w-64 border-2'  value={inputValue} onChange={(e) => setInputValue(e.target.value)}placeholder='Enter handle to see your links' type="text" />
              <button className='cursor-pointer' onClick={()=> handlesearch()}><img src="search.svg" width={50} className='bg-black rounded-full p-2 ml-1' alt="" /></button>
            </div>
        </nav>
            }
            <ToastContainer />
